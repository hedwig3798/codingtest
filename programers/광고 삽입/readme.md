...
### 소감
풀이법을 떠올리는데 오래 걸렸다.   
예전에 풀었던 감시카메라 문제와 비슷한 문제라 생각하여 같은 접근법으로 문제를 풀려 하였지만, 그렇게 된 경우 광고 시간 동안 시청자의 수만 고려하게 되어 문제와 맞지 않았다.   
계속 같은 방향으로 문제를 풀려고 하니 풀이법이 헛돌게 되었다.   
결국 방향을 처음부터 다시 잡아 시간을 초단위로 나누어 풀어보았다.   
영상 시간을 초단위로 나눈 다음 현재 시정자를 누적합 하여 구하고 그것을 다시 누적합 하여 특정 구간 내 사용자의 누적 시청 시간을 구하였다.   
이를 이용하여 광고 시간을 1초씩 뒤로 밀면서 구간합이 가장 큰 구간을 찾았다.
- - -
