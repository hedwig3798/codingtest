...
- - -
### 소감
다익스트라로는 어떻게 문제를 풀어야 할 지 감이 잡히지 않아서 검색을 해 보았다.   
그 결과 와샬 플로이드 알고리즘에 대해 알게 되었고 이 알고리즘으로 해당 문제를 풀 수 있었다.   
아직 다익스트라와 와샬플로이드 알고리즘 모두 숙련도가 낮아서 해당 알고리즘을 활용하는 다른 문제를 더 살펴보고 공부해 봐야겠다.
- - -
