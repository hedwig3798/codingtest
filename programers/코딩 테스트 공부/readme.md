...
- - -
### 소감
문제를 보자 마자 dp문제임을 직감했다.   
dp 배열의 최댓값 선정에서 한번 고민하였다. 처음에는 당연히 150 + 150인 300으로 하면 될 줄 알았지만, 역시나 시간초과에 걸리게 되었다.   
그 다음 어차피 점수의 최댓값 이상인 경우의 최소시간은 그냥 최댓값에 넣으면 되므로 해당 방식으로 문제를 풀었다.   
또한 초기값 설정에 삽질을 오래 해서 문제 푸는 시간이 늘어났다.   
아직 dp문제는 감이 잘 잡히지 않는 것 같다.   
문제를 풀고 보니 다익스트라 알고리즘으로도 풀린다 해서 방법을 한번 찾아봐야겠다. 
- - -
