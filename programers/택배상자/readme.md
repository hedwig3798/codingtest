...
* `order[i]`는 기존의 컨테이너 벨트에 `order[i]`번째 상자를 `i+1`번째로 트럭에 실어야 함을 의미합니다.
- - - 
### 소감
`order`배열의 원소 순으로 택배가 온다 생각하여 풀이 과정이 많이 꼬였다.   
문제를 정확히 인지 한 후에는 큰 문제 없이 풀 수 있었다.   
문제를 정확히 파악하는 능력이 필요해 보인다.
- - -
