...
- - -
### 소감
처음에 트리를 보자 마자 dfs를 이용해서 풀었다.   
하지만 dfs로는 문제가 풀리지 않아서 문제 내용을 다시 살펴보니, 총 판매금의 일부를 주는 것이 아니라 판매한 그 때 마다 판매금의 일부를 위로 올리는 것이였다.   
해당 방법의 경우 굳이 dfs가 필요하지 않을 것 같아서 map을 이용한 풀이 방법을 사용하였고, 정답이였다.   
총 판매금을 주게되는 경우의 코드는 wrong1.cpp이다.
- - -
