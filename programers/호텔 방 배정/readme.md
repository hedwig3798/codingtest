...
- - -
### 소감
map 이랑 hash_map 의 차이를 공부하게 해준 문제.
풀이법 자체는 금방 생각나서 정확성 문제는 금방 풀 수 있었다.   
처음에는 최적화를 하지 않고 union_find 기법을 사용하여서 시간 초과가 나게 되었다.   
그 후 한번 지나간 방 또한 최종적으로 도달하는 방과 연결하는 방식으로 최적화를 하게 되었는데, 그럼에도 시간 초과가 나게 되었다.   
이때 map과 hash_map의 대한 차이를 알게 되었다.   
c++에서 map의 경우 key에 대해 정렬하는 과정을 거치게 되는데 이 때문에 값을 삽입하고 검색하고 수정하는데 시간 복잡도가 높게 된다.   
하지만 hash_map의 경우 hash_table을 사용하기 때문에 시간 복잡도가 map에 비해 낮게 나오게 된다.   
덕분에 자료구조에 대해 다시 복습하게 되었다.
- - -
