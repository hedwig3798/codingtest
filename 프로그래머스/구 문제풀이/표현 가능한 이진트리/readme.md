...
### 소감
dp 방식으로 풀은 문제   
완전이진트리를 만들어야 하므로 문자열의 길이를 완전이진트리로 만들 수 있을 때 까지 앞에 '0'을 붙여 길이를 늘린다.   
그 후 문자열을 나눠가며 만일 한번이라도 제한사항에 맞지 않는 경우 false를 리턴하는 방식으로 풀었다.   
풀이법 자체는 금방 떠올랐지만, 트리 길이를 구하는 방법과 dp 함수 리턴값을 설정하는것에 대해 조금 헤멨다.
- - -
