...
- - -
### 소감   
문제를 풀기 위해 다양한 자료구조를 대입해 보았다.   
처음에는 제한사항을 보고 트리 구조를 만들어 보려고 하였다.   
하지만 구현 도중에 굳이 트리 구조까지 갈 필요가 없다고 생각하여 다른 방법을 고안해 보았다.   
이중 set을 구현할까 했지만 너무 구현이 복잡하였다.   
결국 vector 안에 set을 넣은 구조를 구현하였다.   
해당 구조를 순회하면서 간선이 하나인 노드를 찾고 그 노드와 연결된 다른 모든 노드를 구조체에서 제거하는 방향으로 그리드 알고리즘을 짜서 풀 수 있었다.
- - -
