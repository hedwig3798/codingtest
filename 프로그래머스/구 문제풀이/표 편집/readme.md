...
- - -
### 소감
링크 리스트를 사용해야 한다는 풀이법 자체는 빠르게 떠올랐다.   
하지만 포인터를 이용한 링크리스트를 사용하니 리스트에 삽입 및 삭제시 너무 많은 시간이 소요되어서 간단한 방법으로 구현하였다.   
링크리스트를 벡터로 만들고 실제 삽입 및 삭제가 일어나지는 않도록 구성을 하여 문제를 해결하였다.   
하지만 그럼에도 불구하고 마지막 효율성 문제에서 시간초과가 계속 나와 다른 알고리즘이 있나 고민하였다.   
결국 시간초과의 이유는 벡터 특성 때문에 일어난 일이였다.   
백터가 배열보다 요소 접근 시간이 더 오래 걸리기 때문에 이런 일이 일어났다.   
다음부터는 데이터의 크기가 어느정도 정해진 문제라면 벡터 보단 배열을 사용하도록 노력해야곘다.
- - -
