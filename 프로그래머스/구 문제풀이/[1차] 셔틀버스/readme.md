...
- - -
### 소감
막차만을 고려하는 풀이법은 빠르게 도출 할 수 있었다.
하지만 처음에 stirng 변환에 대해 생각을 너무 많이 하였다.   
stirng 형태를 최대한 보존하고 시간 계산을 하려 하여서 실행 시간과 코드가 복잡해져 오류가 발생해도 그 위치를 모르게 되었다.   
그러한 코드가 worng1.cpp이다.   
결국 복잡도를 줄이기 위해 시간을 최소단위인 분 단위로 변경하고 이를 벡터에 저장하는 식으로 복잡도를 줄였고, 덕분에 틀린 부분이 어디인지와 과도한 string 사용으로 인한 메모리 부족 문제까지 해결되었다.
